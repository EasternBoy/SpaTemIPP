function dstbRetrain!(robo::Vector{robot}, mGP::Vector{GPBase}, NeiB::Vector{Vector{Int64}}, ts::Int64)
    ...
    for i in 1:M
        ...
        # mean   = MeanConst(mGP[i].mean.β)

        kernel = GPtruth.kernel
        mean   = GPtruth.mean
        noise  = GPtruth.logNoise

        mGP[i] = GPE(robo[i].loca, robo[i].obsr, mean, kernel, noise)  

        # GaussianProcesses.optimize!(mGP[i], noisebounds = [-4., 0.])

        if mGP[i].logNoise.value > 0. || mGP[i].logNoise.value < -4.
        ...
        robo[i].rℓ2 = 1 ./mGP[i].kernel.kright.kernel.iℓ2
...
function dstbLinearize_logdet(robo::robot, u::Matrix{Float64}, mGP::GPBase, t::Float64)
    ...
    for r in 1:Dim
        for h in 1:H
            ...
            for k in 1:H
                Ω1[h,k] = Ω1[k,h] = -Cuu[h,k]/robo.rℓ2[r]*(u[r,h] - u[r,k])*
                                        exp(-1/2*abs(k-h)*τ/robo.lℓ)*robo.lσ2
            end

            dCuo = zeros(H, numDa)
            for k in 1:numDa
                dCuo[h,k] = -Cuo[h,k]/robo.rℓ2[r]*(u[r,h] - robo.loca[r,k])*
                                    exp(-1/2*abs(t+h*τ - robo.loca[3,k])/robo.lℓ)*robo.lσ2
            end
    ...
    for i in 1:row
        for j in 1:col
            C[i,j] = robo.lσ2*exp(-1/2*abs(x[3,i] - y[3,j])/robo.lℓ)*
                     robo.rσ2*exp(-1/2*dot(x[1:2,i] - y[1:2,j], diagm(1 ./robo.rℓ2), x[1:2,i] - y[1:2,j]))
        end
    end
